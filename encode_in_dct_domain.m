...
for i=2:last_frame-first_frame+1
    ...
    [height, width, ~] = size(compensated_image_blocks);
    for i2=1:height
       for j2=1:width
           compensated_image_blocks_pixel{i2,j2,i} = idct_transform(compensated_image_blocks{i2,j2,i});
       end
    end
...
for i=2:last_frame-first_frame+1
    ...
    % show image
    imshow(compensated_video_dct(:,:,:,i))
